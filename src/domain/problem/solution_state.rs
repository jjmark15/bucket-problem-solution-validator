...
impl SolutionState {
    ...
    pub(crate) fn apply_action(
        ...
        match action {
            SolutionAction::Pour(pour_action) => self.pour(pour_action),
            SolutionAction::Empty(index) => self.empty_bucket_at_index(*index),
            SolutionAction::Fill(index) => self.fill_bucket_at_index(*index),
        }
...
#[derive(Debug, thiserror::Error)]
#[error("Attempted use of invalid bucket index ({0})")]
