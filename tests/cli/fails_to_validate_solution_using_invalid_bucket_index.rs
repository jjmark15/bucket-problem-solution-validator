...
use crate::helpers::{
    after_error_prefix_starts_with, non_trivial_problem, solution_with_incorrect_bucket_index,
    write_problem_to_file, write_solution_to_file, CliCommandBuilder,
};
...
fn fails_to_validate_solution_using_invalid_bucket_index() {
    ...
    assert.failure().stderr(after_error_prefix_starts_with(
        "Attempted use of invalid bucket index (3)",
    ));
}
